...
> **Formula** :  $f(z) = \overline{z}^2 + c$

The Tricorn fractal is a variant of the Mandelbrot set and is characterized by its triangular shape. It is generated by using a slightly different formula where the complex conjugate of `z` is squared instead of `z` itself. $\overline{z}$ represents the complex conjugate of `z`, and `c` is a complex constant that varies for each pixel in the image.

___
#### Burning Ship Set

The Burning Ship Set is generated by taking the absolute value of the real and imaginary parts before
___
## Implementation 📜
