...
* $\overline{z_n}$ represents the complex conjugate of `z`. 
...
> $f(z_{n+1}) = \left(\left|{Re}(z_n)\right| + i\left|{Im}(z_n)\right|\right)^2 + c$

> $f(z_{n+1}) = \left(\left|{Re}(z_n)\right| + \left|{Im}(z_n)\right|\right i)^2 + c$
...
Again, this formula is a slight modification of the Mandelbrot Set equation but with a significant difference:
