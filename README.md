...
These patterns are generated by:

* Initializing a complex number $z = (x + yi)$ where: $i^2 = -1$
* `x` and `y` are image pixel coordinates mapped to a range from -2 to 2.
* For each pixel in the image:
	* `z` is updated repeatedly following the formula $z = z^2 + c$.
	* `c` is a complex number that seeds a specific Julia set.
	* After `n` iterations, if `z` is less than `2`, the point is considered to be in the Julia set.
	* If after `n` iterations, `z` becomes greater than `2`, the point is considered to have escaped the set. 
	* The color of the pixel is determined by the number of iterations the function plugged with the pixel coordinates reached before blowing out of range.
