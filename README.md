...
> **Formula** :  $f(z_{n+1}) = \overline{z}^2 + c$
...
> $f(z_{n+1}) = \left(\left|\operatorname{Re}(z_n)\right| + i\left|\operatorname{Im}(z_n)\right|\right)^2 + c$
...
Again, this formula is a slight modification of the one used to render the Mandelbrot Set but with a significant difference:
